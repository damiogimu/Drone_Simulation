...
//	set_desire_path2(t, f, des);
	...
	Fx_d = gain->It*intg->area[i][0] + gain->Dt*(des->dxd - x[8]) + gain->Pt*(des->xd - x[0]);
	Fy_d = gain->It*intg->area[i][1] + gain->Dt*(des->dyd - x[9]) + gain->Pt*(des->yd - x[1]);
	...
	U2_d = gain->Ir*intg->area[i][3] + gain->Dr*(des->dad - x[11]) + gain->Pr*(des->ad - x[3]);
	U3_d = gain->Ir*intg->area[i][4] + gain->Dr*(des->dbd - x[12]) + gain->Pr*(des->bd - x[4]);
	U4_d = gain->Ir*intg->area[i][5] + gain->Dr*(des->dcd - x[13]) + gain->Pr*(des->cd - x[5]);
...
int main(void)
{
	...
	gain.Pt = MIN_Pt_GAIN;
	while (gain.Pt <= MAX_Pt_GAIN)
	{
		gain.Dt = MIN_Dt_GAIN;
		while (gain.Dt <= MAX_Dt_GAIN)
		{
			gain.It = MIN_It_GAIN;
			while (gain.It <= MAX_It_GAIN)
			{
				gain.Pr = MIN_Pr_GAIN;
				while (gain.Pr <= MAX_Pr_GAIN)
				{
					gain.Dr = MIN_Dr_GAIN;
					while (gain.Dr <= MAX_Dr_GAIN)
					{
						gain.Ir = MIN_Ir_GAIN;
						while (gain.Ir <= MIN_Ir_GAIN)
						{
							sprintf(filename, "DATA/gain%d_%d_%d_%d_%d_%d", gain.Pt, gain.Dt, gain.It, gain.Pr, gain.Dr, gain.Ir);
							fd = fopen(filename, "w");
							t = 0.0;
							init(state, k, x, &intg, &des);
							while (t <= TIME)
