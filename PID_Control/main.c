...
int func(double t, double *x, double *k, int i, t_rotor *rot, t_desire *des, t_integral *intg, t_param *prm)
{
	double f;
...
//	noncontrol_traj_vt(t, des);
	...
	rot->T4 = U1_d/4.0 + U4_d/(4.0*rot->coefi) - U3_d/(2.0*l);
...
int main(void)
{
	...
	while (t <= TIME+dt)
	{
		...
		while (i < RK4_SIZE)
		{
			...
			func(t, state[i], k[i], i, &rot, &des, &intg, &prm);
			i++;
		}
		j = 0;
		while (j < X_NUM)
