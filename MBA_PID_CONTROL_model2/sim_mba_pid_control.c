...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sim_mba_pid_control.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dmogi <mogi.t218140@gmail.com>             +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/22 17:59:52 by dmogi             #+#    #+#             */
/*   Updated: 2022/12/22 17:59:53 by dmogi            ###   ########.jp       */
/*                                                                            */
/* ************************************************************************** */
...
//----------------------------//
	...
	Fx_d = (m+mp)*(ddxd+Kdt*(dxd-x[8])+Kpt*(xd - x[0])) + mp*r*cos(x[7])*cos(x[6])*(Kdt*(-x[14]) + Kpt*(-x[6])) - mp*r*sin(x[7])*sin(x[6])*(Kdt*(-x[15]) + Kpt*(-x[7])) - mp*r*cos(x[7])*sin(x[6])*x[14]*x[14] - 2*mp*r*sin(x[7])*cos(x[6])*x[14]*x[15] - mp*r*cos(x[7])*sin(x[6])*x[15]*x[15];
	Fy_d = (m+mp)*(ddyd + Kdt*(dyd-x[9])+Kpt*(yd-x[1])) + mp*r*cos(x[7])*(Kdt*(-x[15]) + Kpt*(-x[7])) - mp*r*sin(x[7])*x[15]*x[15];
	Fz_d = (m+mp)*(ddzd+Kdt*(dzd-x[10])+Kpt*(zd-x[2])) + mp*r*cos(x[7])*sin(x[6])*(Kdt*(-x[14])+Kpt*(-x[6])) + mp*r*sin(x[7])*cos(x[6])*(Kdt*(-x[15])+Kpt*(-x[7])) + mp*r*cos(x[7])*cos(x[6])*x[14]*x[14] - 2*mp*r*sin(x[7])*sin(x[6])*x[14]*x[15] + mp*r*cos(x[7])*cos(x[6])*x[15]*x[15] + (m+mp)*g;
	...
	subdata->T3 = T3_d;
	subdata->T4 = T4_d;
	subdata->thrust = subdata->T1 +  subdata->T2 + subdata->T3 + subdata->T4;
...
//-----------  output data  -----------//
		...
		fprintf(fd[6], "%f %f %f %f %f %f\n", t, subdata.T1, subdata.T2, subdata.T3, subdata.T4, subdata.thrust);
