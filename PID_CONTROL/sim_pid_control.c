...
int func(double t, double *x, double *k, int i, t_rotor *rot, t_desire *des, t_integral *intg)
{
	...
	Fx_d = Kix*intg->area[i][0] + Kdx*(des->dxd - x[8]) + Kpx*(des->xd - x[0]);
	Fy_d = Kiy*intg->area[i][1] + Kdy*(des->dyd - x[9]) + Kpy*(des->yd - x[1]);
	...
	U2_d = Kia*intg->area[i][3] + Kda*(des->dad - x[11]) + Kpa*(des->ad - x[3]);
	U3_d = Kib*intg->area[i][4] + Kdb*(des->dbd - x[12]) + Kpb*(des->bd - x[4]);
	U4_d = Kic*intg->area[i][5] + Kdc*(des->dcd - x[13]) + Kpc*(des->cd - x[5]);
