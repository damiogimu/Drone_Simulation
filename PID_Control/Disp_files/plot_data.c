...
int main(void)
{
	...
	double y_min[PDF_NUM+1]={-(amax+1.0), -(vmax+1.0), -1.0, -0.3, -0.6};
	...
	while (i < PDF_NUM)
	{
		...
		if (i == 1)
			fprintf(gp, "set arrow 1 from %f,%f to %f,%f nohead dt (5, 10)\n", 0.0, vmax, TIME, vmax);
		if (i == 2)
			fprintf(gp, "set arrow 1 from %f,%f to %f,%f nohead dt (5, 10)\n", 0.0, 2.0, TIME, 2.0);
		else
		...
		fprintf(gp, "plot [%f:%f][%f:%f]\
				'Data/desire' u 1:%d w l lw 2.0 lc 'red' notitle, \
				\n", X_MIN, TIME, y_min[i], y_max[i], des_data_col[i]);
		i++;
	}
