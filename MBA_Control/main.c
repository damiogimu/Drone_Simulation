...
int func(double t, double *x, double *k, int i, t_rotor *rot, t_desire *des)
{
	...
	double ctm_x, ctm_y, ctm_z, ctm_gamma, ctm_beta;

	f = sqrt(g/r)/(2.0*M_PI);
//	any_traj(t, des);
//	noncontrol_traj_vt(t, des);
	noncontrol_traj_xt(t, des);
	...
	ctm_gamma = Kdt*(-x[14]) + Kpt*(-x[6]);
	ctm_beta = Kdt*(-x[15]) + Kpt*(-x[7]);
	...
	rot->T1 = U1_d/4.0 - U4_d/(4.0*rot->coefi) - U2_d/(2.0*l);
	rot->T4 = U1_d/2.0 - rot->T1 - U2_d/(2.0*l) - U3_d/(2.0*l);
	rot->T2 = U2_d/l + rot->T4;
	rot->T3 = U3_d/l + rot->T1;
...
int main(void)
{
	...
	if (setup(&state, &k, fd, &des, &rot, &gif) == -1)
	{
	...
	while (t <= TIME+dt)
	{
		...
		output_data2(t, x, k, fd, des, rot, gif);
		t += dt;
	}
